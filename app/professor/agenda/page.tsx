                ...
                <div className="space-y-3 min-w-0 flex-1">
                  ...
                  <p className="text-sm md:text-base lg:text-lg text-muted-foreground">
                    ...
                    <div className="flex flex-wrap items-center gap-3 md:gap-6 pt-2">
                      ...
                      <Badge className="bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300 border-green-200 dark:border-green-800 px-2 md:px-3 py-1 text-xs md:text-sm">
                  ...
                  <div className="flex items-center space-x-2 flex-1 lg:flex-none">
                    ...
                    {loadingSemestres || semestres.length === 0 ? (
                      ...
                      <Select value={semestreSelecionado} onValueChange={setSemestreSelecionado}>
                        <SelectTrigger className={`w-full lg:w-40 backdrop-blur-sm ${
                          isLiquidGlass
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 mb-4 md:mb-6">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <h2 className="text-xl md:text-2xl font-bold text-foreground truncate">Grade Horária Semanal</h2>
                ...
                <div className="inline-block min-w-full align-middle px-4 md:px-0">
                  <table className="w-full border-collapse min-w-[600px]">
                    <thead>
                      <tr>
                        ...
                        {getWeekDays.map((day) => (
                          <th key={day.dayName} className="p-2 md:p-3 text-center text-xs md:text-sm font-bold text-foreground border-b border-green-200/50 dark:border-green-800/50">
                            <div className="truncate">{day.dayName}</div>
                            <div className="text-xs text-muted-foreground font-normal truncate">{day.dateStr}</div>
                          </th>
                        ))}
                      </tr>
                    </thead>
                  <tbody>
                    {getHorariosUnicos.length === 0 ? (
                      <tr>
                        <td colSpan={getWeekDays.length + 1} className="p-4 md:p-8 text-center text-xs md:text-sm text-muted-foreground">
              ...
              <div className="relative p-4 md:p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 mb-4 md:mb-6">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <h2 className="text-xl md:text-2xl font-bold text-foreground truncate">Calendário</h2>
                      <p className="text-xs md:text-sm text-muted-foreground truncate">Visualização mensal interativa</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0 w-full sm:w-auto">
                    <Button
                    ...
                    <div className="text-center px-2 md:px-4 flex-1 sm:flex-none">
                ...
                <div className="grid grid-cols-7 gap-1 md:gap-2 mb-2 md:mb-4">
                  {diasSemana.map((dia) => (
                    <div key={dia} className="p-2 md:p-3 text-center">
                      <div className="font-bold text-xs md:text-sm text-foreground truncate">{dia}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-7 gap-1 md:gap-2">
                  {calendarDays.map((dayInfo, index) => (
                    <div
                      key={index}
                      className={`p-1.5 md:p-3 rounded-lg md:rounded-2xl border transition-all duration-300 cursor-pointer group ${
                        dayInfo
                          ? dayInfo.hasEvents
                          ...
                          {dayInfo.hasEvents && (() => {
                            ...
                            return (
                              <div className="space-y-0.5 md:space-y-1">
                                {eventosDoDia.slice(0, 1).map((evento, idx) => (
                                  <div
                                    key={idx}
                                    className={`text-xs rounded-lg px-1 md:px-2 py-0.5 md:py-1 ${getTipoConfig(evento.tipo).bgColor} ${getTipoConfig(evento.tipo).borderColor} border`}
                                  >
                ...
                <div className="mt-4 md:mt-6 pt-4 border-t border-green-200/50 dark:border-green-800/50">
